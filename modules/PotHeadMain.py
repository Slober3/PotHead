...
def crSocketServ(socket_family, socket_type, socket_port, socket_host, socket_max, socket_buff_max,motd, site):
...
#Start server thread   
def runSocketServ(socket_family, socket_type, socket_port, socket_host, socket_max, socket_buff_max,motd,mp,site):
    try:
        if mp == 1:
            print('There are ',len(socket_port)," Ports!")
            for index in range(len(socket_port)):  
               _thread.start_new_thread( crSocketServ, (socket_family, socket_type, int(socket_port[index]), socket_host, socket_max, socket_buff_max,motd,site) )
               print(index,": port ",socket_port[index], " started!")
        else:
               _thread.start_new_thread( crSocketServ, (socket_family, socket_type, socket_port, socket_host, socket_max, socket_buff_max,motd,site) )
